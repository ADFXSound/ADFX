 ...
 * Changelog:
 * v1.6 (2026-09-12)
  + Reduced the default window height for a tighter layout
  + Made the name field use the available content width so it keeps clean right-side padding
 * v1.5 (2026-09-12)
  + Removed the redundant "Name:" label and aligned the name field with the controls below
 * v1.3 (2025-01-13)
...
-- ADFX logo: keep ADFX_O_LOGO.png in the same folder as this script.
local script_path = ({reaper.get_action_context()})[2] or ""
local script_dir = script_path:match("^(.*[\\/])") or ""
local logo_path = script_dir .. "ADFX_O_LOGO.png"
...
function main()
  ...
  local logo = nil
  if ctx and reaper.ImGui_CreateImage then
    local ok_img, img = pcall(reaper.ImGui_CreateImage, logo_path)
    if ok_img and img then
      logo = img
      if reaper.ImGui_Attach then pcall(reaper.ImGui_Attach, ctx, logo) end
    end
  end
  ...
  local config = {
    ...
    window_height = 130, -- Compact height
  ...
  function loop()
    ...
    if visible then
      ...
      -- Full-width input field using the actual content region.
      -- This preserves the window's built-in left/right padding and prevents
      -- the field from running into the right edge.
      local available_width = reaper.ImGui_GetContentRegionAvail(ctx)
      reaper.ImGui_SetNextItemWidth(ctx, math.max(100, available_width))
      ...
      -- Draw the ADFX logo as an overlay in the bottom-right corner.
      -- Using the window draw list means it consumes no ImGui layout space.
      if logo and reaper.ImGui_DrawList_AddImage then
        local wx, wy = reaper.ImGui_GetWindowPos(ctx)
        local ww, wh = reaper.ImGui_GetWindowSize(ctx)
        local logo_size = 32
        local margin = 10
        local x2 = wx + ww - margin
        local y2 = wy + wh - margin
        reaper.ImGui_DrawList_AddImage(
          reaper.ImGui_GetWindowDrawList(ctx), logo,
          x2 - logo_size, y2 - logo_size, x2, y2
        )
      end
