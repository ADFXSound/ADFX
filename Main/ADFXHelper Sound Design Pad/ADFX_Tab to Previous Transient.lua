...
--[[
  ...
  The Sound Design Pad uses ADFX_Tab to Transient.lua for both directions;
  this file does not need to sit next to the pad.
]]
...
-- Run only when this file is the action REAPER launched.
local function launched_as_action()
  ...
  return leaf:lower() == "adfx_tab to previous transient.lua"
end

if launched_as_action() then
  M.prev(reaper, { set_loop = true, enable_repeat = true })
end
